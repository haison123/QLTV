...
namespace QLTV
{
    public partial class frm_acc_xoatk : Form
    {
        account_BUS acc_bus = new account_BUS();
        public frm_acc_xoatk()
        {
            InitializeComponent();
        }

        private void btn_acc_x_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (check == 1)
                    {
                        MessageBox.Show("Xóa tài khoản thành công", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                }
                catch (Exception ex)
                {
